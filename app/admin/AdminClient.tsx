...
import { Users, Shield, Award, CreditCard, RefreshCw, CheckCircle, XCircle, AlertTriangle, Loader2, BookOpen, Plus, Edit, Trash } from "lucide-react";
...
export default function AdminClient({ initialUsers, subscriptions, plans, initialPosts }: AdminClientProps) {
  ...
  const handleDeletePost = async (postId: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Erro ao excluir post");

      // Remove da listagem local
      setPostsList((prev) => prev.filter((post) => post.id !== postId));
      setStatusMessage({ type: "success", text: "Postagem excluída com sucesso." });
    } catch (err: any) {
      setStatusMessage({ type: "error", text: err.message });
    } finally {
      setActionLoading(null);
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-6 border-b border-neutral-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-2">
              <BookOpen className="h-5 w-5 text-emerald-855" />
              <h2 className="font-bold text-neutral-800 text-base">Publicações do Blog ({postsList.length})</h2>
            </div>
            <Link
              href="/admin/blog/novo"
              className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-800 hover:bg-emerald-900 text-white font-bold text-xs px-4 py-3 shadow transition-all active:scale-[0.98]"
            >
          ...
          {postsList.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-neutral-50 border-b border-neutral-150 text-neutral-600 font-bold">
                    <th className="p-4">Capa / Artigo</th>
                ...
                <tbody className="divide-y divide-neutral-100">
                  {postsList.map((post) => (
                    <tr key={post.id} className="hover:bg-neutral-50/50 transition-colors">
                      {/* Imagem de Capa e Detalhes */}
                      <td className="p-4">
                        <div className="flex items-center gap-4">
                          <img 
                            src={post.coverImage} 
                            alt={post.title} 
                            className="w-16 h-10 object-cover rounded-lg border border-neutral-200 shrink-0 bg-neutral-100" 
                          />
                          <div className="space-y-1">
                            <span className="font-bold text-neutral-850 text-sm block">{post.title}</span>
                            <span className="text-[11px] text-neutral-450 block font-mono">slug: {post.slug}</span>
                            <span className="text-[10px] text-neutral-400 block">Criado em: {new Date(post.createdAt).toLocaleDateString("pt-BR")}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        {post.status === "published" ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-800 font-semibold text-[9px] uppercase tracking-wider">
                            Publicado
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-neutral-50 border border-neutral-200 text-neutral-500 font-semibold text-[9px] uppercase tracking-wider">
                            Rascunho
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-right space-x-2">
                        <Link
                          ...
                          className="inline-flex items-center justify-center font-bold px-3 py-1.5 rounded-lg border border-neutral-200 text-[10.5px] hover:bg-neutral-50 text-neutral-700 transition-colors"
                        >
                        ...
                        <button
                          ...
                          disabled={actionLoading !== null}
                          className="inline-flex items-center justify-center font-bold px-3 py-1.5 rounded-lg border border-red-100 text-[10.5px] hover:bg-red-50 text-red-650 transition-colors"
                        >
            ...
            <div className="text-center py-16 space-y-4">
              ...
              <div>
                <span className="font-bold text-neutral-800 text-sm block">Nenhuma postagem no blog</span>
                <span className="text-neutral-500 text-xs mt-1 block">Clique no botão acima para começar a escrever artigos para o portal!</span>
              </div>
            </div>
          )}
        </div>
