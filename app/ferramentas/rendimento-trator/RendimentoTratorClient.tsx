...
export default function RendimentoTratorClient({ isPro, userName }: RendimentoTratorClientProps) {
  ...
  const [larguraTrabalho, setLarguraTrabalho] = useState<number>(4.5); // metros (Ex: largura da grade ou semeadora)
  const [velocidadeTrabalho, setVelocidadeTrabalho] = useState<number>(8.5); // km/h (Velocidade do conjunto)
  const [eficienciaCampo, setEficienciaCampo] = useState<number>(75); // % (Eficiência de campo/manobras)
  const [areaTotal, setAreaTotal] = useState<number>(50); // hectares (Área total a ser trabalhada)
  ...
  const handleGerarPdf = async () => {
    ...
    try {
      ...
      pdf.save(`Rendimento-Operacional-${cliente || "Laudo"}.pdf`);
    } catch (error) {
